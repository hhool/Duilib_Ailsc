...
{
	if(m_PaintManager.FindControl(_T("maxbtn")) || m_PaintManager.FindControl(_T("restorebtn")))
	{
		bHandled = FALSE;
		return 0;
	}
...
BOOL WindowImplBase::IsInStaticControl(CControlUI *pControl, POINT pt)
{
	...
	static std::map<CDuiString, int>::value_type map_init_data[] =
	{
		...
		map<CDuiString, int>::value_type(DUI_CTR_ICON, 2),
		...
		map<CDuiString, int>::value_type(DUI_CTR_QRCODE, 2),
