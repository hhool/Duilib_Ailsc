...
CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1)
{
	...
	memset(m_nColWidth,0,sizeof(int)*UILIST_MAX_COLUMNS);
    Add(m_pHeader);
...
void CListUI::LockProCol(int nLockProCol)
{
	if(m_pList)
		m_pList->LockPreCol(nLockProCol);
}
...
void CListUI::SetPos(RECT rc)
{
	...
	///> 如果锁定了前面N列则前面N列不计算位置
	int nLockProCol = m_pList ? m_pList->GetLockProCol():0;
    for( int i = 0; i < m_ListInfo.nColumns; i++ ) {
		...
		RECT rcPos = pControl->GetPos();
		 m_nColWidth[i] =  pControl->GetFixedWidth();//rcPos.right - rcPos.left;
		if(nLockProCol > 0)
		{
			if(i < nLockProCol)
			{
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("lockprecol")) == 0 ) LockProCol(_ttoi(pstrValue));
...
void CListBodyUI::LockPreCol(int nPrecol)
{
	m_nLockPreCol = nPrecol;
}

int CListBodyUI::GetLockProCol()
{
	return m_nLockPreCol;
}
    ...
    if( cx != 0 && m_pOwner ) {
        ...
        for( int i = 0; i < pInfo->nColumns; i++ ) {
			...
			if(m_nLockPreCol > 0)
			{
				...
				RECT rtPosTemp = pHeader->GetItemAt(i - 1)->GetPos();
			
				rcPos.left -= cx;
				rcPos.right -= cx;
				rcPos.left = rtPosTemp.right;
				rcPos.right = max(rcPos.right,rtPosTemp.right);
				rcPos.right = min(rcPos.right,rcPos.left + m_pOwner->m_nColWidth[i]);
				pControl->SetPos(rcPos);
				pControl->SetMinWidth(0);
				pInfo->rcColumn[i] = pControl->GetPos();
			}
			else
			{
