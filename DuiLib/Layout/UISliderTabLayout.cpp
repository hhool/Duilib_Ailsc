...
{
	return DUI_CTR_SLIDERTABLAYOUT;
}

LPVOID CSliderTabLayoutUI::GetInterface(LPCTSTR pstrName)
{
	if (_tcscmp(pstrName, DUI_CTR_SLIDERTABLAYOUT) == 0) return static_cast<CSliderTabLayoutUI*>(this);
...
bool CSliderTabLayoutUI::SelectItem(int iIndex)
{
    ...
    if(m_pManager != NULL)
    {
		m_pManager->SendNotify(this, DUI_MSGTYPE_TABSELECTED, m_iCurSel, iOldSel);
    }
