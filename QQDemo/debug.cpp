...
//
// RESPONSIBILITIES:
//		Provides automatic locking and unlocking of an OS_CLockable
//object 		within a code scope.
//
// NOTES:
// ............................................................................
class OS_CAutoLock {
...
BOOL DbgPrint(__in LPCTSTR lpszFormatString, ...) {
  ...
  if (g_bSaveLogFile) {
    ...
    if (!s_bLogPathInit) {
      ...
      GetLocalTime(&stime);
#if defined(UNDER_CE)
      _stprintf(s_szLogFile, _T("%s\\log_%04d%02d%02d_%d.log"), g_bLogSavePath,
                stime.wYear, stime.wMonth, stime.wDay, GetTickCount());
#else
      _stprintf_s(s_szLogFile, MAX_PATH, _T("%s\\log_%04d%02d%02d_%d.log"),
                  g_bLogSavePath, stime.wYear, stime.wMonth, stime.wDay,
                  GetTickCount());
#endif
      s_bLogPathInit = true;
    }
    ...
    if (pFile != NULL) {
      ...
      if (cbSize > MAX_LOG_FILE_SIZE) {
        fclose(pFile);
        {
          ...
          GetLocalTime(&stime);
#if defined(UNDER_CE)
          _stprintf(s_szLogFile, _T("%s\\log_%04d%02d%02d_%d.log"),
                    g_bLogSavePath, stime.wYear, stime.wMonth, stime.wDay,
                    GetTickCount());
#else
          _stprintf_s(s_szLogFile, MAX_PATH, _T("%s\\log_%04d%02d%02d_%d.log"),
